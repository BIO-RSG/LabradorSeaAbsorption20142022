...
png("./Figures/05_PowerLawChlaPhytoAbs.png",width=6.5,height=4.5, units = "in",res=300,pointsize = 10 )
...
abline(b = 0.733, a = log10(0.0315), untf=F,col="grey55",lty=3,lwd=2)#Matsuoka et al. (2014) Fig 5b
...
#HPB
slog = rgrlog10(x = plot.dat$chla[plot.dat$regime=="HighPhaeo"], 
                y=  plot.dat$wv443[plot.dat$regime=="HighPhaeo"] )
slog = round(slog,3)
abline(b =slog$Slope, a=slog$Intercept, untf=F,col=in.col[1],lty=2,lwd=2)
dev.off()
rm(plot.dat,slog,in.col,in.col.plot,rgrlog10)